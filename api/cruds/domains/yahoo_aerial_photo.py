...
class YahooAerialPhoto:
  ...
  def get_aerial_photo(self, lat: float, lon: float, zoom: int, save_dir:str):
    url = "https://map.yahooapis.jp/map/V1/static?appid={key}&lat={lat}&lon={lon}&z={zoom}&mode=photo".format(
      key=self.API_KEY,
      lat=str(lat),
      lon=str(lon),
      zoom=str(zoom),
...
if __name__ == '__main__':
  ...
  photo = YahooAerialPhoto(os.environ.get("YAHOO_API_KEY"))
